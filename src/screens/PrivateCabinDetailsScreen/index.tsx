...
const INDUSTRIES = [
  ...
  'Media', 'Retail', 'Real Estate', 'Other',
];
...
export function PrivateCabinDetailsScreen() {
  ...
  const handleSignUp = () => {
    navigation.navigate('PrivateCabinAllSetScreen');
  };
    ...
    <View style={styles.root}>
      <ImageBackground source={BG} style={styles.bg} resizeMode="cover">
        ...
        >
          ...
          >
            <Text style={styles.title}>Private Cabin Details</Text>

            {/* Row 1: Industry + Team Members */}
            <View style={styles.row}>

              {/* Industry dropdown */}
              <View style={styles.halfLeft}>
                <Text style={styles.fieldLabel}>What industry are you in?</Text>
                <View style={styles.dropdownWrapper}>
                  <TouchableOpacity
                    onPress={() => { setShowIndustries(p => !p); setShowMoveIn(false); }}
                    style={styles.dropdown}
                    activeOpacity={0.7}
                  >
                    <Text style={[styles.dropdownValue, !industry && styles.placeholder]}>
                      {industry || 'Industries'}
                    </Text>
                    <Image
                      source={ARROW}
                      style={[styles.arrowIcon, showIndustries && styles.arrowUp]}
                      resizeMode="contain"
                    />
                  </TouchableOpacity>
                  {showIndustries && (
                    <View style={styles.dropList}>
                      {INDUSTRIES.map(ind => (
                        <TouchableOpacity
                          key={ind}
                          style={[styles.dropItem, ind === industry && styles.dropItemActive]}
                          onPress={() => { setIndustry(ind); setShowIndustries(false); }}
                          activeOpacity={0.7}
                        >
                          <Text style={[styles.dropItemText, ind === industry && styles.dropItemTextActive]}>
                            {ind}
                          </Text>
                        </TouchableOpacity>
                      ))}
              ...
              <View style={styles.halfRight}>
                <Text style={styles.fieldLabel}>How many team members?</Text>
                <View style={styles.counter}>
                  <TouchableOpacity
                    onPress={() => setTeamMembers(n => Math.max(1, n - 1))}
                    style={[styles.counterBtn, teamMembers <= 1 && styles.counterBtnOff]}
                    disabled={teamMembers <= 1}
                    activeOpacity={0.7}
                  >
                    <Text style={styles.counterBtnText}>−</Text>
                  </TouchableOpacity>
                  <Text style={styles.counterVal}>{teamMembers}</Text>
                  <TouchableOpacity
                    onPress={() => setTeamMembers(n => n + 1)}
            ...
            <View style={styles.dropdownWrapper}>
              <TouchableOpacity
                onPress={() => { setShowMoveIn(p => !p); setShowIndustries(false); }}
                style={styles.dropdown}
                activeOpacity={0.7}
              >
            ...
            <View style={styles.tourRow}>
              <TouchableOpacity
                onPress={() => {
                  setBookTour(p => !p);
                  if (!bookTour) setShowCalendar(false);
                }}
              ...
              {bookTour && (
                <View style={styles.tourDateTimeRow}>
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                  >
                    <Text style={styles.datePillText}>{tourDateLabel()}</Text>
                  </TouchableOpacity>
                  <View style={styles.timePill}>
                    <Text style={styles.datePillText}>{selectedTime}</Text>
                  </View>
                </View>
              )}

              {/* Show placeholder date/time when not booked */}
              {!bookTour && (
                <View style={styles.tourDateTimeRow}>
                  <View style={[styles.datePill, styles.datePillDim]}>
                    <Text style={styles.datePillTextDim}>Jun 10, 2024</Text>
                  </View>
                  <View style={[styles.timePill, styles.datePillDim]}>
                    <Text style={styles.datePillTextDim}>9:41 AM</Text>
                  </View>
                </View>
              )}
            </View>
...
const styles = StyleSheet.create({
  ...
  counterBtnText: { color: Colors.white, fontSize: 16, fontWeight: 'bold' },
  counterVal:     { fontFamily: 'SequelSans-MediumBody', fontSize: 15, color: Colors.textPrimary },
